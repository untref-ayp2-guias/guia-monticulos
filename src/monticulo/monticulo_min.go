...
// de forma ordenada. En este caso, se implementa un montículo mínimo.
type MonticuloMin[T constraints.Ordered] struct {
	// Elementos del montículo
	elementos []T
}

// NewMonticulo crea un nuevo montículo vacío.
func NewMonticuloMin[T constraints.Ordered]() *MonticuloMin[T] {
	return &MonticuloMin[T]{}
}

// Len devuelve el número de elementos del montículo.
func (m *MonticuloMin[T]) Len() int {
	return len(m.elementos)
}

// Insertar inserta un nuevo elemento en el montículo.
func (m *MonticuloMin[T]) Insertar(elemento T) {
...
// Costo: O(log n)
func (m *MonticuloMin[T]) upHeap(i int) {
	for i > 0 {
		padre := (i - 1) / 2
...
// BorrarMin extrae el elemento mínimo del montículo.
func (m *MonticuloMin[T]) BorrarMin() (T, error) {
	var elemento T
	if m.Len() == 0 {
...
// Costo: O(log n)
func (m *MonticuloMin[T]) downHeap(i int) {
	for {
		izq := 2*i + 1
		der := 2*i + 2
		if izq >= m.Len() {
		...
		if m.elementos[i] <= m.elementos[min] {
			break
		}
		m.elementos[i], m.elementos[min] = m.elementos[min], m.elementos[i]
		i = min
	}
}
